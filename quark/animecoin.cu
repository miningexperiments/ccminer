...
// Original Animehash Funktion aus einem miner Quelltext
extern "C" void animehash(void *state, const void *input)
{
	...
	sph_bmw512 (&ctx_blake, input, 80);
	sph_bmw512_close(&ctx_blake, (void*) hash);

	sph_blake512_init(&ctx_bmw);
	sph_blake512 (&ctx_bmw, (const void*) hash, 64);
	sph_blake512_close(&ctx_bmw, (void*) hash);
...
#define TRACE(algo) { \
	if (max_nonce == 1 && pdata[19] <= 1) { \
		...
		printf("anime %s %08x %08x %08x %08x...%08x... \n", algo, swab32(debugbuf[0]), swab32(debugbuf[1]), \
...
extern "C" int scanhash_anime(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
...
// cleanup
extern "C" void free_anime(int thr_id)
{
	int dev_id = device_map[thr_id];
	if (!init[thr_id])
		return;
